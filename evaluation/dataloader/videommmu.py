...
class VideoMMMU_Bench:
    ...
    def get_data(self):
        ...
        for category_name in self.category:
            path = os.path.join(self.data_dir, category_name, 'test-00000-of-00001.parquet')
    ...
    def process_data(self, doc):
        ...
        text_input = videommmu_doc_to_text(doc, think_mode=self.think_mode)
...
def videommmu_doc_to_text(doc, think_mode=False):
    category_name = doc["category"]
    text_input = videoperception_doc_to_text_no_preprompt(doc)
    ...
    think_prompt = "You must first think about the reasoning process in the mind and then provide the user with the answer. The reasoning process and answer are enclosed within <think> </think> and <answer> </answer> tags, respectively. All reasoning must be grounded in visual evidence from the video. When you mention any related object, person, or specific visual element, you must strictly follow the following format: `<obj>object_name</obj><box>bounding_box</box>at<t>time_in_seconds</t>s`. The reasoning process MUST NOT be longer than 100 words. In the answer part, respond with only the letter of the correct option."
