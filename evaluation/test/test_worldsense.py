...
from dataloader.worldsense import (
    WorldSense_Bench,
    dump,
    worldsense_process_result,
        ...
        def process_batch(batch_video_paths, batch_text_input, batch_image, batch_doc):
            ...
            for path_idx in range(N):
                ...
                if think_mode:
                    ...
                    ans = parse_multi_choice_response(pred_text)
                    if ans in ["A","B","C","D"]:
                        pred_list.append(ans)
                        score = 1.0
                    else:
...
def evaluate(args, num_gpus, gpu_list):
    ...
    vis_dir = "./logs/world_logs"
    if not os.path.exists(vis_dir):
        os.makedirs(vis_dir)
    result_file_base = f"{args.exp_name}.xlsx"
