...
from dataloader.videommmu import (
    VideoMMMU_Bench,
    videoperception_aggregate_results,
    videoperception_process_results,
    videommmu_doc_to_text,
    parse_options,
    parse_answer
)
import argparse
...
def evaluate_chunk(video_paths, image, text_input, docs, gpu_id, args, queue):
    ...
    try:
        ...
        def process_batch(batch_video_paths, batch_text_input, batch_image, batch_doc):
            ...
            question_type = batch_doc[0].get("question_type", "None")
            if question_type != "multiple-choice":
                mcq = "In the answer part, respond with only the letter of the correct option."
                batch_text_input[0] = batch_text_input[0].replace(mcq, "In the answer part, respond with only the final answer.")

                output_list, _, _ = model(
                    batch_video_paths,
                    batch_text_input,
                ...
                for path_idx in range(N):
                    ...
                    if think_mode:
                        ...
                        if match:
                            ...
                            if ans not in ["A","B","C","D","E","F","G","H","I","J"]:
                                score = 0
                                pred_list.append("NA")
                ...
                choice_score = {"A": 0, "B": 0, "C": 0, "D": 0, "E": 0, "F": 0, "G":0, "H":0, "I":0, "J":0}
                for i in range(N):
                    if pred_list[i] == "NA":
                        continue
        ...
        metrics = [
            videoperception_process_results(docs[i], results[i])
...
def evaluate(args, num_gpus, gpu_list):
    ...
    video_paths, image, text_input, docs = videommmu.get_data()
    ...
    infer_end = time.time()
    print(
        f"Average inference time per example: {(infer_end - infer_start) / total:.4f} seconds"
    )

    acc = videoperception_aggregate_results(all_metrics)
    print("Final accuracy:", acc)

    videommmu.cal_category_acc(all_metrics)
