...
qwen_processor = VideoLlavaProcessor.from_pretrained("LanguageBind/Video-LLaVA-7B-hf")
...
def inject_prompt(message, choices):
    ...
    # new_sys_prompt = 'Directly choose one correct choice from 0, 1, 2 or 3. No explanation.\n'
    new_sys_prompt = 'Answer with ONLY a single digit (0, 1, 2, or 3). Do not include any other text.\n\n'
...
def generate_video_llava_answer(model, processor, messages):
    """Generate answer from Video-LLaVA using same message format as Qwen."""
    try:
        ...
        # Video-LLaVA expects at least 8 frames, duplicate if needed
        if images:
            min_frames = 8
            ...
            # Text only
            prompt = f"USER: {text_prompt} ASSISTANT:"
            inputs = processor(
                text=prompt,
                return_tensors="pt"
            ).to(model.device, torch.bfloat16)
        ...
        print(f"\n❌ Error in generate_video_llava_answer: {e}")
        import traceback
        traceback.print_exc()
        raise e

def eval_video_llava(dataset, model, processor, mcq_dict):
    ...
    print(f"\n[Video-LLaVA Eval] Evaluating on {num_examples} examples...")
    print("=" * 80)
    correct = 0
    ...
    for i in tqdm(range(num_examples)):
        ...
        try:
            ...
            generated_text = generate_video_llava_answer(model, processor, messages)
...
print("Final Accuracy:", eval_video_llava(eval_dataset, model, processor, mcq_dict))