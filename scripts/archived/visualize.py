...
def load_original_labels(episode: str, season: int, labels_dir: Path) -> Optional[Dict]:
    ...
    for label_file in season_dir.glob("*.json"):
        if episode_lower in label_file.name.lower():
            try:
                with open(label_file, 'r') as f:
                    # print("label file ", label_file, "season, ", season, "episode, ", episode)
                    labels = json.load(f)
                return labels
            except:
                continue
...
def extract_category_frames(
    ...
    model, processor = load_model("Qwen/Qwen2-VL-2B-Instruct", ckpt_pth)
    model2, processor2 = load_model("Qwen/Qwen2-VL-2B-Instruct", None)
    ...
    print(f"\nFound entries:")
    for cat, entries in target_categories.items():
    ...
    for category, entries in target_categories.items():
        ...
        for idx, item in enumerate(selected_entries[:frames_per_category], 1):
            ...
            if question in search_dict.keys():
                ...
                message = dataset[message_idx]
                answer = answer_question(model, processor, message)
                answer2 = answer_question(model2, processor2, message)
            else:
            ...
            print(f"      Q: {question}...")
            print(f"      A (OURS): {answer}")
            print(f"      A (BASELINE): {answer2}")
            print(f"      REF: {gt}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt-pth", type=str, default=None)
