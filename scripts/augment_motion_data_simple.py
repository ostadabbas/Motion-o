...
def compute_direction_and_speed(bboxes: List[List[float]], 
                                timestamps: List[float]) -> Tuple[str, float]:
    """
    Compute dominant direction and average speed from bbox trajectory.
    
    Returns:
        (direction_str, avg_speed)
    """
    if len(bboxes) < 2:
    ...
    return (direction, avg_speed)


def compute_smoothness(bboxes: List[List[float]], 
                       timestamps: List[float]) -> str:
    """
    Compute trajectory smoothness quality.
    
    Returns:
        "smooth", "jerky", or "erratic"
    """
    if len(bboxes) < 3:
        return "smooth"
    ...
    # Compute variance in speeds (smoothness indicator)
    ...
    cv = std_dev / mean_speed if mean_speed > 0 else 0.0
...
def generate_motion_text(bboxes: List[List[float]], 
                         timestamps: List[float]) -> str:
    """Generate natural language motion description."""
    ...
    direction, avg_speed = compute_direction_and_speed(bboxes, timestamps)
    
    if direction == "stationary":
        return "stationary (no significant motion)"
    
    # Compute smoothness
    quality = compute_smoothness(bboxes, timestamps)
