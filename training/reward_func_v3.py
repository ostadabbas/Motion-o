...
# from training.motion_reward_v2 import motion_trajectory_reward
from training.motion_reward_v3 import motion_trajectory_reward, motion_grounding_reward
...
# ============================================================================
# Valid attribute values for v2 discrete motion tags
# ============================================================================
VALID_MOTION_DIR = {"STAT", "N", "NE", "E", "SE", "S", "SW", "W", "NW"}
VALID_MOTION_SPEED = {"stationary", "slow", "moderate", "fast"}
VALID_MOTION_SCALE = {"approaching", "stable", "receding"}
VALID_MOTION_SHAPE = {"linear", "curved", "oscillating", "stationary"}
VALID_MOTION_PHASE = {"accelerating", "constant", "decelerating", "reversing"}

# Pattern for self-closing v2 motion tags: <motion obj="x" dir="E" .../>
MOTION_V2_PATTERN = re.compile(r'<motion\s+[^>]*?/>')


def _count_valid_v2_motion_tags(think_content: str) -> int:
    """Count v2 self-closing motion tags with valid attribute values."""
    valid = 0
    for tag in MOTION_V2_PATTERN.findall(think_content):
        dir_m = re.search(r'dir="([^"]*)"', tag)
        speed_m = re.search(r'speed="([^"]*)"', tag)
        scale_m = re.search(r'scale="([^"]*)"', tag)
        # Core 3 attributes must be present and valid
        if not (dir_m and speed_m and scale_m):
            continue
        if (dir_m.group(1) not in VALID_MOTION_DIR or
            speed_m.group(1) not in VALID_MOTION_SPEED or
            scale_m.group(1) not in VALID_MOTION_SCALE):
            continue
        # Optional extended attributes: penalise if present but invalid
        shape_m = re.search(r'shape="([^"]*)"', tag)
        phase_m = re.search(r'phase="([^"]*)"', tag)
        if shape_m and shape_m.group(1) not in VALID_MOTION_SHAPE:
            continue
        if phase_m and phase_m.group(1) not in VALID_MOTION_PHASE:
            continue
        valid += 1
    return valid
...
def format_reward(completions, **kwargs):
    ...
    for content in completion_contents:
        ...
        # v2: count self-closing motion tags (no paired open/close needed)
        count_motion_v2 = _count_valid_v2_motion_tags(think_content)
        # v1 legacy: still check paired <motion>...</motion> during transition
        count_motion_v1_open = think_content.count('<motion>')
        count_motion_v1_close = think_content.count('</motion>')
        is_motion_v1_paired = (count_motion_v1_open == count_motion_v1_close)
        ...
        # 如果数目不匹配，直接返回0
        if not (is_obj_paired and is_time_paired and is_box_paired and is_motion_v1_paired):
            rewards.append(0.0)
            continue
        ...
        # Motion-tag enforcement for motion-relevant tasks:
        # for temporal-spatial and general video QA tasks, require at least one valid motion tag
        # Accepts EITHER v2 self-closing tags OR v1 legacy paired tags
        ...
        total_valid_motion = count_motion_v2 + count_motion_v1_open
        if requires_motion and total_valid_motion == 0:
