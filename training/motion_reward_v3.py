...
# ============================================================================
# Motion Grounding Reward — Dual-Chain Visual Verification (NEW in v3)
# ============================================================================

def motion_grounding_reward(completions, **kwargs):
    """
    Measures whether the model's motion predictions are grounded in actual
    video observation vs text-based anticipation.
    
    Compares each rollout's <motion/> tags against a masked-video output
    (where motion evidence was removed by freezing frames).
    
    If predictions CHANGE when evidence is removed → grounded (reward=1)
    If predictions STAY THE SAME → not grounded (reward=0)
    """
    rewards = []
    
    # Get the masked completion (generated from frozen-frame video)
    masked_completions = kwargs.get('masked_completion', [None])
    masked_text = masked_completions[0] if masked_completions else None
    
    if masked_text is None:
        return [0.0] * len(completions)
    
    # Parse motion tags from masked output (once)
    masked_think = re.search(r"<think>(.*?)</think>", masked_text, re.DOTALL)
    masked_tags = parse_v2_motion_tags(masked_think.group(1)) if masked_think else {}
    ...
    W_D, W_S, W_C = 0.5, 0.3, 0.2  # direction matters most for grounding
    
    for completion in completions:
        if task not in motion_tasks:
            rewards.append(0.0)
            continue
        
        think_match = re.search(
            r"<think>(.*?)</think>", completion[0]["content"], re.DOTALL
        )
        if not think_match:
            rewards.append(0.0)
            continue
        
        pred_tags = parse_v2_motion_tags(think_match.group(1))
        
        if not pred_tags:
            rewards.append(0.0)
            continue
        
        # Compare each object's motion tags
        per_obj = []
        for obj_name, pred_attrs in pred_tags.items():
            m_attrs = masked_tags.get(obj_name, {})
            
            if not m_attrs:
                # Object wasn't mentioned in masked output → strong grounding
                per_obj.append(1.0)
                continue
            
            d_changed = float(pred_attrs.get("dir", "") != m_attrs.get("dir", ""))
            s_changed = float(pred_attrs.get("speed", "") != m_attrs.get("speed", ""))
            c_changed = float(pred_attrs.get("scale", "") != m_attrs.get("scale", ""))
            
            per_obj.append(W_D * d_changed + W_S * s_changed + W_C * c_changed)
        
        rewards.append(float(np.mean(per_obj)) if per_obj else 0.0)
