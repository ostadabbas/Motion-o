...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # ============================================================
        # v3: Masked-chain generation for motion grounding reward
        # ============================================================
        masked_completion_text = None
        task_name = inputs[0].get('task', '')
        motion_tasks = {
        ...
        if task_name in motion_tasks and multi_image and image_inputs is not None:
            try:
                # 1. Create frozen frames (duplicate first frame forward)
                if isinstance(image_inputs, list) and torch.is_tensor(image_inputs[0]):
                    original_frames = image_inputs[0]
                else:
                    original_frames = image_inputs[0] if isinstance(image_inputs, list) else image_inputs
                
                frozen_frames = original_frames.clone()
                # Freeze: duplicate frame 0 across all positions
                # This removes all motion signal while keeping the scene
                key_frames_data = inputs[0].get('key_frames', [])
                if key_frames_data and video_inputs is not None:
                    fps = video_kwargs.get('fps', [1.0])[0]
                    first_kf_time = key_frames_data[0].get('time', 0)
                    freeze_idx = min(max(0, int(first_kf_time * fps)), len(frozen_frames) - 1)
                else:
                    freeze_idx = 0
                
                freeze_frame = frozen_frames[freeze_idx].clone()
                for fi in range(freeze_idx + 1, len(frozen_frames)):
                    frozen_frames[fi] = freeze_frame
                
                # 2. Build masked inputs (same text prompt, frozen video)
                masked_prompt_inputs = self.processing_class(
                    text=copy.deepcopy(prompts_text),
                    images=[frozen_frames],
                    videos=None,
                    return_tensors="pt",
                    padding=True,
                    padding_side="left",
                ...
                if self.max_prompt_length is not None:
                    ...
                    masked_prompt_inputs["attention_mask"] = masked_prompt_inputs["attention_mask"][:, -self.max_prompt_length:]
                
                # 3. Generate ONE greedy output (no gradient, deterministic)
                masked_gen_config = GenerationConfig(
                    max_new_tokens=self.max_completion_length,
                    do_sample=False,
                ...
                with torch.no_grad():
                    with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                        masked_output_ids = unwrapped_model.generate(
                            **masked_prompt_inputs,
                            generation_config=masked_gen_config,
                        )
                
                masked_prompt_len = masked_prompt_inputs["input_ids"].size(1)
                masked_completion_ids = masked_output_ids[:, masked_prompt_len:]
                masked_completion_text = self.processing_class.batch_decode(
                    masked_completion_ids, skip_special_tokens=True
                )[0]
                
            except Exception as e:
                print(f"[v3 grounding] Masked chain failed: {e}")
                masked_completion_text = None
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            # v3: Pass masked completion to grounding reward
            reward_kwargs['masked_completion'] = [masked_completion_text] * len(prompts)
        ...
        # v3: Track whether masked chain ran successfully
        self._metrics["grounding/has_masked_chain"].append(1.0 if masked_completion_text is not None else 0.0)
